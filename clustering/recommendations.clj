...
(defn sum-up [prefs fun person shared-items]
   (reduce + (map (fn [shared-item] (fun ((prefs person) shared-item)) 
   shared-items)))

; figure out how to calculate pSum
(defn sim-pearson [prefs p1 p2]
   "Returns a pearson correlation-based similarity score for p1 and p2"
   (let [p1-prefs (prefs person1)
         p2-prefs (prefs person2)
         items1 (set (keys p1-prefs))
         items2 (set (keys p2-prefs))
         shared-items (intersection items1 items2)]
      (if (empty? shared-items)
         0
         (let [sum1 (sum-up prefs (fn [retval] retval) person1 shared-items)
               sum2 (sum-up prefs (fn [retval] retval) person2 shared-items)
               sum1Sq (sum-up prefs (fn [val] (Math/pow val 2)) person1 shared-items)
               sum2Sq (sum-up prefs (fn [val] (Math/pow val 2)) person2 shared-items)
               pSum (sum-up prefs )]))))